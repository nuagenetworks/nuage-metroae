#!/usr/bin/python

from ansible.module_utils.basic import AnsibleModule
try:
    from vspk.v4_0 import NUVSDSession
    HAS_VSPK = True
except ImportError:
    HAS_VSPK = False

DOCUMENTATION = '''
---
module: get_gw_purge_time
short_description: Get VSD gateway purge time parameter
options:
  vsd_auth:
    description:
      - VSD credentials to access VSD GUI
    required: true
    default: null
  get_gateway_purge_time:
    description:
      - Extract VSD gateway purge time
    required: true
    default: true
'''

EXAMPLES = '''
# Extract gateway purge time in VSD
- config_vsd_system:
    vsd_auth:
      username: csproot
      password: csproot
      enterprise: csp
      api_url: https://10.0.0.10:8443
    get_gateway_purge_time: true
'''


def get_vsd_session(vsd_auth):
    try:
        session = NUVSDSession(**vsd_auth)
        session.start()
        csproot = session.user
        return csproot
    except:
        module.exit_json(changed=False, result="Could not establish\
                          connection to VSD")


arg_spec = dict(
    vsd_auth=dict(required=True, type='dict'),
    get_gateway_purge_time=dict(required=True, type='bool')
)
module = AnsibleModule(argument_spec=arg_spec, supports_check_mode=True)
if not HAS_VSPK:
        module.fail_json(msg='vspk is required for this module')


def main():
    vsd_auth = module.params['vsd_auth']
    get_gateway_purge_time = module.params['get_gateway_purge_time']
    csproot = get_vsd_session(vsd_auth)
    if get_gateway_purge_time is True:
        try:
            sys_config = csproot.system_configs.get_first()
            purge_val = sys_config.ad_gateway_purge_time
        except Exception as e:
            module.fail_json(msg="Could not retrieve\
                         gateway purge timer : %s" % e)
    module.exit_json(changed=True,
                     result=purge_val)


main()
