...
from vspk.v4_0 import NUNSGateway, NUVSDSession, NUSubnet
import subprocess
import yaml
import sys
...
def get_zone_obj(csp_user, org_name = 'Nuage_Partition1', l3_domain_name = 'oc-heat-test'):
    csproot = csp_user
    ...
    if l3_domain.name != l3_domain_name:
        ...
        sys.exit(1)
    zone = l3_domain.zones.get_first(filter="name=='%s'" % vsd_constants['zone_name'])
...
def delete_subnet(network_name, zone):
    ...
    if network.name != network_name:
        print("ERROR: Could not delete %s network on VSD or network does not exist" % network_name)
        sys.exit(1)
    network.delete()
...
def create_subnet(zone):
    ...
    #TODO add a check for oversubscription of network 
...
if __name__ == '__main__':
    ...
    parser.add_argument("delete_subnet", type=bool,default=False,nargs='?',
                        help="Set to True if subnet needs to delete. Default is false")
    ...
    try:
        ...
        with open(args.playbook_dir + '/roles/ci-predeploy/vars/main.yml', 'r') as fo:
    ...
    # Get zone
    zone_obj = get_zone_obj(csproot,vsd_constants['org_name'],vsd_constants['domain_name'])
