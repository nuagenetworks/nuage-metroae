...
# note 4: in case you use redundant DC-GWs you also need to provide routing policies (manually at bgp level) to avoid routing loops. See also ACG.

# example of metadata to be added in VSD WAN Service: "rddc=1:30000,rdwan=10.0.0.1:3000,rtwan=65000:3000"
...
# teardown example cli: tools perform service vsd evaluate-script domain-name "l2dom1" type l2-domain action teardown policy "py-l2-I-ES" vni 1234 rt-i target:1:1 rt-e target:1:1
...
# remove trailing space at the end of the metadata
    ...
    if servicetype == "L2DOMAIN":
      ...
      if not rtwan.startswith ('target'):
         rtwan = "target:"+rtwan
      print ('servicetype, VPLS id, rtdc, vni, rddc, rdwan, rtwan:', servicetype, vplsSvc_id, rtdc, vni, rddc, rdwan, rtwan)
      dyn.add_cli("""
        configure service
