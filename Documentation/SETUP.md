...
```
You can optionally specify the data directory path. If you don't specify it on the command line, you will be prompted to enter it. This path is required for container operation. The data directory is the place where docs, examples, and your deployment files will be kept and edited. It is also the place where Nuage Networks image and package files will be pulled from. Note that setup will create subdirectories beneath the directory you specify as `metroae_data`. For example, if you specify `/tmp` for data directory path during setup, setup will create `/tmp/metroae_data` for you. Setup will copy docs, logs, and deployment files to `/tmp/metroae_data`. Inside the container itself, setup will mount `/tmp/metroae_data` as `/data/`. Therefore, when you specify path names for metroae when using the container, you should always specify the container-relative path. For example, if you copy your tar.gz files to `/tmp/metroae_data/images/6.0.1` on the host, this will appear as `/data/images/6.0.1` inside the container. When you use the unzip-files action on the container, then, you would specify a source path as `/data/images/6.0.1`. When you complete the nuage_unzipped_files_dir variable in common.yml, you would also specify `/data/images/6.0.1`. Note that you can run setup multiple times and setup will not destroy or modify the data you have on disk. If you specify the same data directory that you had specified on earlier runs, metroae will pick up the existing data. Thus you can update the container as often as you like and your deployments will be preserved.
