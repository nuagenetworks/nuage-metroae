...
When QCOW2 files are pre-positioned, you must add a command-line variable, 'skipCopyImages', to indicate that copying QCOW2 files should be skipped. Otherwise, the QCOW2 files will be copied again. An extra-vars 'skipCopyImages' needs to be passed on the command line during the deployment phase to skip copying of the image files again. For example, to pre-position the QCOW2 images, run:
...
```
metroae install_everything.yml --extra-vars skipCopyImages=True
```
