...
* If you are running MetroAE via the MetroAE container, you can unzip these files using the metroae script that you installed via RPM and used to setup the container and get it running. During the setup, you were promoted for the location of an data directory on the host. This data directory is mounted in the container as `/data`. Therefore, for using the unzip action, you must 1) copy your tar.gz files to a directory under the directory you specified at setup time and 2) you must specify a container-relative path on the unzip command line. For example, if you specified the data directory as `/tmp`, setup created the directory `/data/metroae_data` on your host and mounted that same directory as `/data` in the container. Assuming you copied your tar.gz files to `/tmp/metroae_data/6.0.1` on the docker host, your unzip command line would be as follows: `metroae unzip-files /data/6.0.1/ /data/6.0.1` For details on the installation of the RPM, see [SETUP.md](SETUP.md).
