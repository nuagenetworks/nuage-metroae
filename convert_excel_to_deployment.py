            ...
            if value is not None:
                if label is not None:
                    ...
                    if (worksheet.title == 'Nsgvs' and cell.coordinate == "B49"):
                        print("WARNING: NSG Infra profile template name has been changed to NSG template name. Please update your excel spreadsheet to be compatible with latest metroae version. You can find excel spreadsheet in the metro github repo.") 
            else:
                self.cell_positions[label] = cell.coordinate
            if fields_by_col:
                col += 1
